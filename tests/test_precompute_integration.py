...
def test_eth_cache_roundtrip():
    ...
    pe._cache["0xabc"] = {"ts": time.time(), "data": entry}
...
def test_sol_cache_roundtrip():
    ...
    ps._cache["obligation_abc"] = {"ts": time.time(), "data": entry}
...
def test_eth_cache_eviction():
    ...
    pe._cache.clear()
    pe._last_block = 200
    pe._cache["0xhot"] = {"ts": time.time(), "data": {"updated_block": 199}}  # 1 block old
    pe._cache["0xcold"] = {"ts": time.time(), "data": {"updated_block": 195}}  # 5 blocks old
...
def test_sol_cache_eviction():
    ...
    ps._cache["hot_obl"] = {"ts": time.time(), "data": {"updated_slot": 440333099}}  # 1 slot old
    ps._cache["cold_obl"] = {"ts": time.time(), "data": {"updated_slot": 440333050}}  # 50 slots old
